...
- cta 필드는 카드 하단에 파란 글씨로 표시되는 보조 문구다. 마지막(CTA) 카드에서는 "프로필 링크를 통해 상담 받아보세요"처럼 서비스 확인·상담 유도 문구로 쓰고(전화번호·이메일 직접 노출 금지, 06-content-rules.md 참고), 중간 카드에서는 그 카드에 인용한 **외부** 수치의 출처로 쓴다.
- 중간 카드에 company-facts.md의 "외부 리서치를 인용한 수치"(Zendesk, SQM Group, Splunk, 맥킨지 등)를 썼다면, 그 카드의 cta에 반드시 출처를 남긴다. 예: `"cta": "(출처: SQM Group, 2025)"`. 반면 "개별 고객사 사례"(A/B/C/F/K사 등 CS쉐어링 자체 사례) 수치는 우리 회사 자료이므로 cta에 출처를 넣지 않는다 — cta는 빈 문자열로 둔다. 인용할 만한 검증된 수치가 없으면 수치 자체를 만들지 않는다.
...
## 길이
title·subtitle·item 글자수 제한은 06-content-rules.md를 따른다(모든 카드 title 15자/줄, subtitle 25자, 중간 카드 title 줄바꿈 1번 등). item.body는 길게 해설하지 않고 박스 안에 들어가는 짧은 설명형 문장으로 작성한다. 1번 카드 subtitle은 빈 문자열("")이 아닌 실제 소제목을 반드시 작성해야 한다.
