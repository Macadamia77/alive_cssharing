...
async function generateContent(
  ...
  // 인스타그램은 JSON만 출력해야 하므로 HTML 카드 가이드를 보내지 않음
  // (MD 가이드와 충돌해서 JSON이 잘리거나 파싱 실패하는 원인이었음)
...
위 초안의 핵심 메시지와 방향성을 유지하면서, 채널 가이드에 맞게 완성해주세요.${imageCardGuide}`
  ...
  if (provider !== "mock") {
    ...
    // 인스타그램은 JSON이 길어 토큰 부족으로 잘릴 수 있어 maxTokens 증가
...
// ─── 코드 블록 제거 헬퍼 ─────────────────────────────────────
// AI가 ```json ... ``` 또는 ```html ... ``` 로 감싸서 반환하는 경우 제거
// 닫는 펜스 뒤에 설명 텍스트가 있어도 올바르게 처리
function stripCodeFence(text: string): string {
  const s = text.trim();
  const lines = s.split('\n');
  if (!lines[0]?.match(/^(?:```|~~~)[\w-]*\s*$/)) return s;
