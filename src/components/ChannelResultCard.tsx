...
export default function ChannelResultCard({ channel, status, content, stage, cardAssets }: Props) {
  ...
  // 네이버 블로그 HTML에서 이미지 카드(<figure>)만 따로 추출 — "발행 준비" 패널의 ZIP 다운로드용.
  const cards = useMemo(
    () => (channel === "naver-blog" && content ? extractCards(content) : []),
    [channel, content]
  );
  ...
  // "발행 준비" ① — 이미지 태그를 뺀 순수 본문 텍스트만 복사(네이버 에디터 붙여넣기용)
