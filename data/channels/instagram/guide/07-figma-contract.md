...
- cta 필드는 카드 하단에 파란 글씨로 표시되는 보조 문구다. 마지막(CTA) 카드에서는 실제 행동 유도 문구(연락처 등)로 쓰고, 중간 카드에서는 그 카드에 인용한 수치의 출처로 쓴다.
- 중간 카드의 title/subtitle/highlight_text/items 어디든 "N%"나 "N배" 같은 구체적 성과 수치를 썼다면, 그 카드의 cta에 반드시 출처를 남긴다. 예: `"cta": "(출처: cssharing.com 인사이트 - 고객센터 반복문의, 인력 아닌 구조로 줄이는 법)"`. 수치를 인용하지 않은 중간 카드는 cta를 빈 문자열로 둔다 — 억지로 채우지 않는다.
...
## 길이
- 모든 카드(1번/중간/CTA)에서 title은 한 줄당 최대 15자 이내(공백 포함), subtitle은 줄바꿈 없이 1줄, 최대 25자 이내로 작성한다.
- 중간 카드 title은 반드시 2줄이다. 줄바꿈(\n)은 정확히 1번만 사용한다. 3줄 이상 절대 금지. 각 줄은 위 15자 제한을 지킨다. 예시: "비용 부담과\n운영 난이도"
- 중간 카드 subtitle은 줄바꿈 없이 1줄, 최대 25자 이내다.
- 1번 카드 subtitle은 빈 문자열("")이 아닌 실제 소제목을 반드시 작성해야 한다. title을 보완하는 소제목, 줄바꿈 없이 최대 25자 이내.
- 1번 카드 title도 위 15자/줄 제한을 지킨다. 필요하면 중간 카드처럼 줄바꿈 1번으로 2줄까지 쓸 수 있다.
