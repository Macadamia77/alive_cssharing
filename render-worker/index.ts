...
// run_id 하나당 결과 아카이브를 한 번만 시도하게 막는 가드. 이 값은 워커 프로세스가 1개라는
// 전제(Railway에 이 서비스가 단일 인스턴스로 배포됨)에서만 유효하다 — 워커를 여러 인스턴스로
// 늘리게 되면 DB 레벨의 멱등 키(예: results.run_id + UNIQUE 제약)로 바꿔야 한다.
const archivedRuns = new Set<string>();

// [결과 유실 방지] 지금까지는 프론트엔드가 폴링하다가 완료를 감지한 시점에만 결과를
// results 테이블에 저장했다 — naver-blog처럼 오래 걸리는 채널은 프론트가 자기 타임아웃(12분)
// 으로 먼저 포기해버리면, 워커가 나중에 실제로 완성해도 그 결과를 저장할 사람이 아무도 없어
// 그대로 유실됐다(실제로 재현·확인함). 워커가 작업을 종료 처리하는 시점에 직접 저장하면
// 프론트가 보고 있든 말든 완성된 결과는 항상 남는다.
async function archiveRunResultsIfSettled(runId: string): Promise<void> {
  if (archivedRuns.has(runId)) return;

  const { data: siblings, error } = await supabase
    .from("tasks")
    .select("channel, status, result, card_assets")
    .eq("run_id", runId)
    .eq("job_type", "generate");
  if (error || !siblings || siblings.length === 0) return;

  const allSettled = siblings.every((t) => t.status === "completed" || t.status === "failed");
  if (!allSettled) return;
  if (archivedRuns.has(runId)) return; // 동시에 여러 채널이 거의 같이 끝나는 경우의 이중 저장 방지
  ...
  for (const t of siblings) {
    if (t.status === "completed" && t.result) {
      channelsMap[t.channel] = t.result;
      if (t.card_assets && t.card_assets.length > 0) cardAssetsMap[t.channel] = t.card_assets;
    }
  }
  if (Object.keys(channelsMap).length === 0) return; // 전부 실패면 저장할 게 없음(프론트 로직과 동일)

  const { data: run } = await supabase
    .from("brainstorm_runs")
    .select("selected_topic, user_draft")
    .eq("id", runId)
    .maybeSingle();
  const topic = (run?.selected_topic || (run?.user_draft || "").slice(0, 40) || "").trim() || "제목 없음";

  const now = new Date();
  const id = `${now.toISOString().replace(/[-:T]/g, "").slice(0, 14)}-${Math.random().toString(36).slice(2, 6)}`;
  const { error: saveErr } = await supabase.from("results").insert({
    id,
    topic,
    channels: channelsMap,
    created_at: now.toISOString(),
    card_assets: Object.keys(cardAssetsMap).length > 0 ? cardAssetsMap : null,
  });
  if (saveErr) console.error(`[Worker] run ${runId} 결과 아카이브 실패:`, saveErr.message);
...
// 강제로 failed 처리한다.
// 20분 — 예전엔 10분이었는데, naver-blog 검수 재시도가 겹치는 실제 케이스가 10분을 넘겨
// 정상 진행 중인 작업이 이 워치독에 의해 잘못 죽을 위험이 있었다(2026-07-11 실측: 정상 완료까지
...
async function processTask(task: any) {
  ...
  try {
    ...
    if (task.run_id) void archiveRunResultsIfSettled(task.run_id);
  } catch (e: any) {
